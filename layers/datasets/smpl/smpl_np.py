...
class SMPLModel():
	def __init__(self, model_path, rest_pose=np.zeros(72)):
		...
		"""
		print('----- Intialize SMPL Model -----')
		self.rest_pose = rest_pose.reshape((-1, 1, 3))
		print('rest_pose shape:', self.rest_pose.shape)
		with open(model_path, 'rb') as f:
			...
			print('J_regressor shape:', self.J_regressor.shape)
			self.weights = params['weights']
			print('weights shape:', self.weights.shape)
			self.posedirs = params['posedirs']
			print('posedirs shape:', self.posedirs.shape)
			self.v_template = params['v_template']
			print('v_template shape:', self.J_regressor.shape)
			# self.shapedirs = params['shapedirs']
			self.shapedirs = params['shapedirs'][:, :, :10]
			print('shapedirs shape:', self.shapedirs.shape)
			self.faces = np.int32(params['f'])
			print('faces shape:', self.faces.shape)
			self.kintree_table = params['kintree_table']
			print('kintree_table shape:', self.kintree_table.shape)
