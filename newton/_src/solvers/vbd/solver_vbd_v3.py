...
from .particle_vbd_kernels import (
    ...
    update_velocity,
    compute_blocked_displacement,
    propagate_blocked_displacement,
)
...
class SolverVBD(SolverBase):
    ...
    def _init_particle_system(
        ...
        self.particle_blocked_displacements = wp.zeros(
            model.particle_count, dtype=wp.vec3, device=self.device
        )
        self.particle_received_displacements = wp.zeros(
            model.particle_count, dtype=wp.vec3, device=self.device
        )
        ...
        if self.particle_enable_self_contact:
            ...
            wp.launch(
                ...
                inputs=[
                    ...
                    self.inertia,
                    self.particle_received_displacements,
                ],
    ...
    def solve_particle_iteration(self, state_in: State, state_out: State, contacts: Contacts, dt: float, iter_num: int, max_iter_num=-1):
        ...
        self.particle_received_displacements.zero_()
        if data_collector.is_log_collision():
        ...
        if self.particle_enable_self_contact:
            # Step 1: blocked displacement 계산 (전체 particle)
            # color 구분 없이 전체 particle에 대해 계산
            
            wp.launch(
                kernel=compute_blocked_displacement,
                dim=self.model.particle_count,
                inputs=[
                    self.model.particle_flags,
                    state_in.particle_q,    # x_actual (truncation 후 현재 위치)
                    self.inertia,           # x_free (contact 없이 갔을 위치)
                    self.model.particle_mass,
                ],
                outputs=[self.particle_blocked_displacements,],
                device=self.device,
            )

            # Step 2: contact pair 기반으로 이웃에게 blocked 전달/propagation
            # → particle_received_displacements 업데이트
            wp.launch(
                kernel=propagate_blocked_displacement,
                dim=self.particle_self_contact_evaluation_kernel_launch_size,
                inputs=[
                    self.model.tri_indices,
                    self.model.edge_indices,
                    self.trimesh_collision_info,
                    state_in.particle_q,                # 현재 위치 (normal 계산용)
                    self.particle_blocked_displacements,# 이전 iteration 값 사용
                    0.3,                                # alpha: 처음엔 작게 시작
                    self.trimesh_collision_detector.edge_edge_parallel_epsilon,
                ],
                outputs=[self.particle_received_displacements,],
                device=self.device,
            )
            
            # Step 3: received_displacement는 다음 iteration의 
            # accumulate_self_contact_force_and_hessian에서 apply_conservative_bound_truncation에서 자동으로 사용됨
