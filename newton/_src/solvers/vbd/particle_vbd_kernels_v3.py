...
``solver_vbd_v3.py`` and call into functions defined here.
"""
...
@wp.kernel
def compute_blocked_displacement(
    particle_flags: wp.array(dtype=wp.int32),
    pos_actual: wp.array(dtype=wp.vec3),        # actual displacement (truncation 후)
    inertia: wp.array(dtype=wp.vec3),           # x_free 추정값
    mass: wp.array(dtype=float),
    # output
    particle_blocked_displacements: wp.array(dtype=wp.vec3),
):
    particle_index = wp.tid()

    if not particle_flags[particle_index] & ParticleFlags.ACTIVE or mass[particle_index] == 0:
        particle_blocked_displacements[particle_index] = wp.vec3(0.0)
        return

    # x_free: contact 없이 갔을 위치 = inertia (elasticity solve 결과)
    # blocked = x_free - x_actual
    x_actual = pos_actual[particle_index]   # 절대 위치 (state_in.particle_q)
    x_free = inertia[particle_index]        # 절대 위치
    
    particle_blocked_displacements[particle_index] = x_free - x_actual  # 둘 다 절대 위치라서 차이가 변위


@wp.kernel
def propagate_blocked_displacement(
    tri_indices: wp.array(dtype=wp.int32, ndim=2),
    edge_indices: wp.array(dtype=wp.int32, ndim=2),
    collision_info_array: wp.array(dtype=TriMeshCollisionInfo),
    pos: wp.array(dtype=wp.vec3),
    particle_blocked_displacements: wp.array(dtype=wp.vec3),  # 이전 iteration 값
    alpha: float,  # propagation strength (0.0 ~ 1.0)
    edge_edge_parallel_epsilon: float,
    # output
    particle_received_displacements: wp.array(dtype=wp.vec3),
):
    t_id = wp.tid()
    ...
    # vertex-triangle contact pair 처리
    if primitive_id < collision_info.vertex_colliding_triangles_buffer_sizes.shape[0]:
        ...
        while collision_buffer_counter < collision_info.vertex_colliding_triangles_buffer_sizes[primitive_id]:
            ...
            if particle_idx != -1 and tri_idx != -1:
                ...
                # contact normal 계산 (vertex → triangle 방향)
                closest_p, _bary, _feature_type = triangle_closest_point(a, b, c, p)
                diff = p - closest_p
                dis = wp.length(diff)

                if dis > 1e-8:
                    n = diff / dis  # vertex에서 triangle 쪽으로의 normal

                    # vertex의 blocked displacement 중 normal 방향 성분만 triangle vertices에 전달
                    blocked_v = particle_blocked_displacements[particle_idx]
                    transfer_v = wp.dot(blocked_v, -n) * (-n)  # triangle 방향 성분
                    transfer_v = alpha * transfer_v

                    wp.atomic_add(particle_received_displacements, tri_a, transfer_v)
                    wp.atomic_add(particle_received_displacements, tri_b, transfer_v)
                    wp.atomic_add(particle_received_displacements, tri_c, transfer_v)

                    # triangle vertices의 blocked displacement 중 normal 방향 성분을 vertex에 전달
                    blocked_tri_avg = (
                        particle_blocked_displacements[tri_a]
                        + particle_blocked_displacements[tri_b]
                        + particle_blocked_displacements[tri_c]
                    ) / 3.0
                    transfer_tri = wp.dot(blocked_tri_avg, n) * n
                    transfer_tri = alpha * transfer_tri

                    wp.atomic_add(particle_received_displacements, particle_idx, transfer_tri)

            collision_buffer_counter += NUM_THREADS_PER_COLLISION_PRIMITIVE

    # edge-edge contact pair 처리
        ...
        while collision_buffer_counter < collision_info.edge_colliding_edges_buffer_sizes[primitive_id]:
            ...
            if e1_idx != -1 and e2_idx != -1:
                ...
                st = wp.closest_point_edge_edge(
                    e1_v1_pos, e1_v2_pos, e2_v1_pos, e2_v2_pos, edge_edge_parallel_epsilon
                )
                s = st[0]
                t_ee = st[1]
                c1 = e1_v1_pos + (e1_v2_pos - e1_v1_pos) * s
                c2 = e2_v1_pos + (e2_v2_pos - e2_v1_pos) * t_ee
                diff = c1 - c2
                dis = st[2]

                if dis > 1e-8:
                    n = diff / dis  # e1에서 e2 방향의 normal

                    bs_e1 = wp.vec2(1.0 - s, s)
                    bs_e2 = wp.vec2(1.0 - t_ee, t_ee)

                    # e1의 blocked → e2로 전달
                    blocked_e1_avg = (
                        particle_blocked_displacements[e1_v1] * bs_e1[0]
                        + particle_blocked_displacements[e1_v2] * bs_e1[1]
                    )
                    transfer_to_e2 = alpha * wp.dot(blocked_e1_avg, -n) * (-n)

                    wp.atomic_add(particle_received_displacements, e2_v1, transfer_to_e2 * bs_e2[0])
                    wp.atomic_add(particle_received_displacements, e2_v2, transfer_to_e2 * bs_e2[1])

                    # e2의 blocked → e1로 전달
                    blocked_e2_avg = (
                        particle_blocked_displacements[e2_v1] * bs_e2[0]
                        + particle_blocked_displacements[e2_v2] * bs_e2[1]
                    )
                    transfer_to_e1 = alpha * wp.dot(blocked_e2_avg, n) * n

                    wp.atomic_add(particle_received_displacements, e1_v1, transfer_to_e1 * bs_e1[0])
                    wp.atomic_add(particle_received_displacements, e1_v2, transfer_to_e1 * bs_e1[1])
...
def forward_step_penetration_free(
    ...
    pos[particle_index] = apply_conservative_bound_truncation(
        particle_index, pos_inertia, pos_prev_collision_detection, particle_conservative_bounds, particle_received_displacements
    )
...
def apply_conservative_bound_truncation(
    ...
    pos_prev_collision_detection: wp.array(dtype=wp.vec3),
    particle_conservative_bounds: wp.array(dtype=float),
    particle_received_displacements: wp.array(dtype=wp.vec3),  # float 아니라 vec3
):
    ...
    received = particle_received_displacements[v_index]
    received_norm = wp.length(received)

    if received_norm > 1e-8:
        received_dir = received / received_norm
        normal_proj = wp.dot(accumulated_displacement, received_dir)
        normal_component = normal_proj * received_dir
        tangent_component = accumulated_displacement - normal_component

        relaxed_bound = conservative_bound + received_norm
        tangent_norm = wp.length(tangent_component)

        # received 방향 성분 clamp
        clamped_normal = normal_component
        if normal_proj > relaxed_bound:
            clamped_normal = relaxed_bound * received_dir

        # tangent 방향 성분 clamp
        clamped_tangent = tangent_component
        if tangent_norm > conservative_bound and conservative_bound > 1e-5:
            clamped_tangent = tangent_component * (conservative_bound / tangent_norm)

        accumulated_displacement = clamped_normal + clamped_tangent
        return particle_pos_prev_collision_detection + accumulated_displacement

    else:
        # received 없으면 기존 scalar 방식
        ...
        if accumulated_displacement_norm > conservative_bound and conservative_bound > 1e-5:
            accumulated_displacement = accumulated_displacement * (
                conservative_bound / accumulated_displacement_norm
            )
        return particle_pos_prev_collision_detection + accumulated_displacement
...
def solve_trimesh_with_self_contact_penetration_free(
    ...
    if abs(wp.determinant(h)) > 1e-5:
        ...
        pos_new[particle_index] = apply_conservative_bound_truncation(
            particle_index, particle_pos_new, pos_prev_collision_detection, particle_conservative_bounds, particle_received_displacements
        )
