...
# ---------------------------------------------------------------------------
# Planar-DAT: parallel-by-collision implementation (upstream newton-physics)
# arXiv:2604.15513  —  division-plane helpers + atomic-min kernels
# ---------------------------------------------------------------------------
...
def segment_plane_intersects(
    ...
    d: wp.vec3,
    eps_parallel: float,
    eps_intersect_near: float,
    eps_intersect_far: float,
    coplanar_counts: bool,
) -> bool:
    """Return True if the segment [v, v+delta_v] crosses the plane dot(n, p-d)=0."""
    nv = wp.dot(n, delta_v)
    num = -wp.dot(n, v - d)
...
def robust_edge_pair_normal(
    ...
    """Robust normal for a (possibly degenerate) edge-edge pair."""
...
def create_vertex_triangle_division_plane_closest_pt(
    ...
    """Compute the division plane for a vertex-triangle pair.

    n points toward the vertex side.  Returns (is_dummy[4], n, d).
    is_dummy[i] = True means vertex i does not need truncation.
    """
...
def create_edge_edge_division_plane_closest_pt(
    ...
    """Compute the division plane for an edge-edge pair.

    n points toward the e0 side.  Returns (is_dummy[4], n, d).
    """
    ...
    if wp.length(n_hat) < 1e-12:
        return (
            ...
            robust_edge_pair_normal(e0_v0_pos, e0_v1_pos, e1_v0_pos, e1_v1_pos, 1.0e-6),
            c1 * 0.5 + c2 * 0.5,
        )
    n = wp.normalize(n_hat)
    delta_e0 = wp.max(
    ...
    return (
        wp.vector(
            is_dummy_for_e0_v0, is_dummy_for_e0_v1, is_dummy_for_e1_v0, is_dummy_for_e1_v1,
            length=4, dtype=wp.bool,
        ),
        n,
        d,
    )
...
def planar_truncation_t(
    ...
    """Compute the truncation ratio t in [0,1] for vertex v with displacement delta_v.

    t=1 means no truncation.  The plane is dot(n, p - d) = 0.
    """
...
def apply_planar_truncation_parallel_by_collision(
    ...
    """For each collision pair, compute per-vertex truncation ratios via atomic_min.

    Processes both EE and VT pairs in a single parallel pass.  Each thread
    handles one entry in one primitive's collision buffer.
    """
    t_id = wp.tid()
    collision_info = collision_info_array[0]
        ...
        while collision_buffer_counter < collision_info.edge_colliding_edges_buffer_sizes[primitive_id]:
            ...
            if e1_idx != -1 and e2_idx != -1:
                ...
                is_dummy, n, d = create_edge_edge_division_plane_closest_pt(
                    pos[e1_v1], displacement_in[e1_v1],
                    pos[e1_v2], displacement_in[e1_v2],
                    pos[e2_v1], displacement_in[e2_v1],
                    pos[e2_v2], displacement_in[e2_v2],
                )
                if not is_dummy[0]:
                    wp.atomic_min(truncation_t_out, e1_v1,
                                  planar_truncation_t(pos[e1_v1], displacement_in[e1_v1], n, d, parallel_eps, gamma, 0.0))
                if not is_dummy[1]:
                    wp.atomic_min(truncation_t_out, e1_v2,
                                  planar_truncation_t(pos[e1_v2], displacement_in[e1_v2], n, d, parallel_eps, gamma, 0.0))
                if not is_dummy[2]:
                    wp.atomic_min(truncation_t_out, e2_v1,
                                  planar_truncation_t(pos[e2_v1], displacement_in[e2_v1], n, d, parallel_eps, gamma, 0.0))
                if not is_dummy[3]:
                    wp.atomic_min(truncation_t_out, e2_v2,
                                  planar_truncation_t(pos[e2_v2], displacement_in[e2_v2], n, d, parallel_eps, gamma, 0.0))
            collision_buffer_counter += NUM_THREADS_PER_COLLISION_PRIMITIVE

    # --- Vertex-triangle pairs ---
    if primitive_id < collision_info.vertex_colliding_triangles_buffer_sizes.shape[0]:
        particle_idx = primitive_id
        v_pos = pos[particle_idx]
        v_disp = displacement_in[particle_idx]
        collision_buffer_counter = t_id_current_primitive
        collision_buffer_offset = collision_info.vertex_colliding_triangles_offsets[primitive_id]
        while collision_buffer_counter < collision_info.vertex_colliding_triangles_buffer_sizes[primitive_id]:
            tri_idx = collision_info.vertex_colliding_triangles[
            ...
            if particle_idx != -1 and tri_idx != -1:
                ...
                is_dummy, n, d = create_vertex_triangle_division_plane_closest_pt(
                    v_pos, v_disp,
                    pos[tri_a], displacement_in[tri_a],
                    pos[tri_b], displacement_in[tri_b],
                    pos[tri_c], displacement_in[tri_c],
                )
                if not is_dummy[0]:
                    wp.atomic_min(truncation_t_out, particle_idx,
                                  planar_truncation_t(v_pos, v_disp, n, d, parallel_eps, gamma, 0.0))
                if not is_dummy[1]:
                    wp.atomic_min(truncation_t_out, tri_a,
                                  planar_truncation_t(pos[tri_a], displacement_in[tri_a], n, d, parallel_eps, gamma, 0.0))
                if not is_dummy[2]:
                    wp.atomic_min(truncation_t_out, tri_b,
                                  planar_truncation_t(pos[tri_b], displacement_in[tri_b], n, d, parallel_eps, gamma, 0.0))
                if not is_dummy[3]:
                    wp.atomic_min(truncation_t_out, tri_c,
                                  planar_truncation_t(pos[tri_c], displacement_in[tri_c], n, d, parallel_eps, gamma, 0.0))
            collision_buffer_counter += NUM_THREADS_PER_COLLISION_PRIMITIVE
...
def apply_truncation_ts(
    ...
    """Apply per-vertex truncation ratios and isotropic displacement cap.

    Final position: pos_out[i] = pos[i] + clamp(t * displacement_in[i], max_displacement).
    """
    i = wp.tid()
    t = truncation_ts[i]
...
def compute_displacement(
    ...
    """displacement_out[i] = pos_new[i] - pos_ref[i]."""
    i = wp.tid()
    displacement_out[i] = pos_new[i] - pos_ref[i]
