...
class Example:
    def __init__(self, viewer, args=None):
        data_collector.set_record_name('MJ_SMPL_2layers_00084_again')
        # frame timing
        self.fps = 60
        ...
        self.sf = 1.0
        # garments parameter
        g_visible_list = [0, 1] # start from 0
        g_tri_ke = [5.0e2, 1.0e3]
        g_tri_ka = [5.0e2, 1.0e3]
        g_tri_kd = [2.0e-4, 2.0e-3]
        g_edge_ke = [1.0e2, 2.0e2]
        g_edge_kd = [1.0e-3, 1.0e-2]
        g_density = [0.2, 0.4]
        g_rigid_particle_radius = [0.05 * self.sf, 0.10 * self.sf]
        solver_particle_self_contact_radius = 0.05*self.sf
        solver_particle_self_contact_margin = 0.10*self.sf
        solver_rigid_contact_k_start = 5.0e5
        model_soft_contact_ke = 1.0e4
        model_soft_contact_kd = 1.0e-4
        model_soft_contact_mu = 0.2
        # kinematic human
        human_hidden = False
        human_slowdown_factor = 10
        ...
        self.human_mesh = SMPLMesh(
            ...
            sub_steps=self.sim_substeps * human_slowdown_factor, 
        ...
        # scale
        uniform_sf = wp.vec3(self.sf,self.sf,self.sf)

        # position
        v = self.human_mesh.curr_pos.numpy()
        human_mean_v = wp.vec3(v.mean(axis=0)) * self.sf
        human_minz_v = v[:, 2].min() * self.sf
        human_maxz_v = v[:, 2].max() * self.sf
        ...
        for i, g in enumerate(infos['garment']):
            ...
            scene.add_cloth_mesh(
                ...
                tri_ke=g_tri_ke[i],
                tri_ka=g_tri_ka[i],
                # tri_kd=2.0e-3, # initial
                tri_kd=g_tri_kd[i],
                # tri_kd=2.0e0,
                edge_ke=g_edge_ke[i],
                # edge_kd=1e-1,
                edge_kd=g_edge_kd[i],
                density=2*g_density[i],
                scale=uniform_sf[0],
                vertices=gv,
                indices=gf,
                # particle_radius=5e-3
                particle_radius=g_rigid_particle_radius[i] # for rigid-particle collision
        ...
        self.viewer._cam_speed = 4.0 * self.sf
        print()
        ...
        self.cloth_solver = newton.solvers.SolverVBD(
            ...
            particle_self_contact_radius=solver_particle_self_contact_radius, # particle
            particle_self_contact_margin=solver_particle_self_contact_margin, # yk: this handles the entire contact margin. (= query radius)
            ...
            rigid_contact_k_start=solver_rigid_contact_k_start,
            # ogc_contact=True
        )

        # self.model.soft_contact_ke = 2.0e7
        # self.model.soft_contact_kd = 1.0e-1 # initial
        self.model.soft_contact_ke = model_soft_contact_ke
        self.model.soft_contact_kd = model_soft_contact_kd # initial
        # self.model.soft_contact_kd = 1.0e1 # modified
        self.model.soft_contact_mu = model_soft_contact_mu
...
if __name__ == "__main__":
    ...
    # 1) 로그 모드 켜기 (충돌 로깅이면)
    data_collector.log_mode = 1  # 또는 LOG_PERFORMANCE

    # 2) 저장 폴더 + run 이름 만들기
    out_dir = "/home/user/Desktop/newton/newton/logs"
    os.makedirs(out_dir, exist_ok=True)

    run_name = f"{out_dir}/smpl2layer_{time.strftime('%Y%m%d_%H%M%S')}"
