...
class Example:
    def __init__(self, viewer, args=None):
        ...
        cloth_size = 100
        xs = np.linspace(-50.0, 50.0, cloth_size)
        zs = np.linspace(-50.0, 50.0, cloth_size)
        vertices = [wp.vec3(float(xs[j]), 0.0, float(zs[i]))
                    for i in range(cloth_size) for j in range(cloth_size)]
        mesh_indices = np.array(
            [idx
             for i in range(cloth_size - 1)
             for j in range(cloth_size - 1)
             for idx in [i*cloth_size+j,       (i+1)*cloth_size+j,   i*cloth_size+j+1,
                         (i+1)*cloth_size+j,   (i+1)*cloth_size+j+1, i*cloth_size+j+1]],
            dtype=np.int32,
        )
