    ...
    def __init__(self, viewer, args=None):
        ...
        for i in range(0,50):
            scene.add_cloth_mesh( # yk: how to add cloth mesh is written here
                pos=wp.vec3(0.0, 0.0, 0.9+i*0.01), #wp.vec3(i*0.02, i*0.02, 1.2+i*0.05),
        ...
        scene.add_shape_cylinder(
            body=-1,
            xform=wp.transform(p=wp.vec3(0.0, -0.1, 0.6), q=wp.quat_from_axis_angle(wp.vec3(0, 1, 0), wp.pi / 2)),
            radius=0.25
        )
        # plane
...
if __name__ == "__main__":
    ...
    # 1) 로그 모드 켜기 (충돌 로깅이면)
    data_collector.log_mode = 0 #data_collector.LOG_COLLISION_INFO  # 또는 LOG_PERFORMANCE
