...
def apply_rotation(
    ...
    angular_constant = 5.0

    if cur_t  * angular_velocity > 2.0 * angular_constant * math.pi:
        return

    # # Rotate back and forth
    # if cur_t * angular_velocity < math.pi:
    #     theta = cur_t * angular_velocity 
    # else:
    #     if (((cur_t * angular_velocity - math.pi) // (2.0 * math.pi)) % 2.0) == 0:
    #         theta = theta - ((cur_t * angular_velocity) % (2.0 * math.pi))
    #     else:
    #         theta = theta + (cur_t * angular_velocity % (2.0 * math.pi))

    #Rotate one way
    theta = cur_t * angular_velocity
    if cur_t * angular_velocity < angular_constant * math.pi:
        theta = cur_t * angular_velocity
    else:
        theta = 2.0 * angular_constant * math.pi - cur_t * angular_velocity
