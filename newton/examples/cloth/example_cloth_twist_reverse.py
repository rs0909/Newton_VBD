...
from .penetration_debug import SelfPenetrationDetector, PenetrationViewerDebug
...
class Example:
    def __init__(self, viewer, args=None):
        ...
        self.pen_detector = SelfPenetrationDetector(eps=1e-7)
        self.pen_debug = PenetrationViewerDebug(self.viewer, device="cpu")
    ...
    def render(self):
        cloth_v = self._get_cloth_vertices_numpy()
        cloth_f = self._get_cloth_faces_numpy()
        pen = self.pen_detector.compute(cloth_v, cloth_f)

        self.viewer.begin_frame(self.sim_time)
        self.viewer.log_state(self.state_0)
        self.pen_debug.log(pen)
        self.viewer.end_frame()
    ...
    def _get_cloth_vertices_numpy(self) -> np.ndarray:
        return self.state_0.particle_q.numpy().astype(np.float32)

    def _get_cloth_faces_numpy(self) -> np.ndarray:
