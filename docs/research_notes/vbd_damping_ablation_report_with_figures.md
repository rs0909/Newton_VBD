...
**Figure 4.** Scene B (`frictionless_sliding`) mesh snapshots at t=0.00s, 0.25s, and 0.48s (full variant, iter=100). Blue = top layer (free, 625 vertices), red/salmon = bottom layer (625 vertices, pinned at its four corners). At t=0 the two layers are nearly flat and separated by 0.04m. By t=0.25s both layers have sagged under gravity into a bowl shape, and the top (blue) layer has already settled into the bottom of the bowl formed by the pinned bottom (red) layer — i.e., self-contact has engaged. By t=0.48s the configuration is essentially static, matching the near-zero `end_of_step/kinetic_energy` (≈1.28J) reported in §4.1. The viewing angle makes the blue layer mostly hidden behind/inside the red layer's bowl, which is itself evidence of the sustained contact between the two layers described in §2.4 ("Sustained, ~400 pairs").
